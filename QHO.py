...
class Wavepacket : 
    ...
    def __init__(self, N=600, L=100., dt=0.1, periodic=True):
        ...
        self.V_width = 2.0*math.sqrt(self.E * L)  # width of potential at energy level we set
    ...
    def V(self, x):
        ...
        if self.gaussian:
            return self.V_0 * ( (x-self.V_center)*(x-self.V_center) + math.pow(x-self.V_center,4.0))
        else:
